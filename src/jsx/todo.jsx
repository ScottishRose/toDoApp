...
class Todo extends React.Component {
  ...
  render() {
    let todoItems = this.state.todoItems.map((todoItem, index) => {
      return (
        <li className={'todo-items_item' + (todoItem.complete ? ' todo-items_item-complete' : '')} key={index}>
          <button onClick={() =>
            this.handleCheck(index) } className="todo-items_check-btn">&#10004; </button>
          <button onClick={() =>
            this.handleDelete(index)} className="todo-items_delete-btn">&#x2716; </button>
    ...
    return (
      <div>
        <input placeholder= "What do you need to do?" onChange={this.handleChange} onKeyDown={this.handleKeyDown} />
